...
describe('TickTickService', () => {
	...
	describe('ensureVaultFilesRegistered - misplaced file location conflicts', () => {
		...
		it('should register without a project association when keepProjectFolders is disabled', async () => {
			(getSettings as Mock).mockReturnValue({ keepProjectFolders: false });

			await service.ensureVaultFilesRegistered();

			// This fork never infers a file's owning project from its filename
			// when keepProjectFolders (the explicit opt-in) is off.
			expect(upsertFile).toHaveBeenCalledWith('ProjectA.md');
			expect(upsertFile).not.toHaveBeenCalledWith('ProjectA.md', 'proj-a');
